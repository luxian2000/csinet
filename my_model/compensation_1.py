...
class QuantumCompensationBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 为了效率，批量处理可能较慢，这里逐个处理
        for i in range(num_patches):
            patch = patches[:, :, i]  # [batch, ch*4]
            
            # 将patch reshape为 [batch, ch, 4]
            patch = patch.reshape(batch, ch, 4)
            
            # 对每个通道独立应用量子补偿
            qc_out_channels = []
            for c in range(ch):
                # patch_c shape: [batch, 4]
                patch_c = patch[:, c, :]
                
                # 对batch中的每个样本应用量子电路
                batch_outputs = []
                for b in range(batch):
                    # 将输入归一化到[-π, π]范围（RY门的输入范围）
                    input_data = torch.tanh(patch_c[b]) * np.pi
                ...
                # 堆叠batch维度的输出
                qc_out = torch.stack(batch_outputs)  # [batch, n_qubits]
                qc_out_channels.append(qc_out)
            
            # 合并通道 [batch, ch, n_qubits]
            channel_output = torch.stack(qc_out_channels, dim=1)
            compensated_patches.append(channel_output)
        
        # 将所有patches的结果拼接 [batch, num_patches, ch, n_qubits]
        compensated = torch.stack(compensated_patches, dim=1)
...
class QuantumCompensatedDecoder(nn.Module):
    ...
    def forward(self, s):
        ...
        # 应用量子补偿
        comp = self.quantum_comp(comp)  # [batch, 8, 16, 16] (2通道×4量子比特)
...
# ==================== 数据加载 ====================
def load_data(envir='indoor', data_path='data'):
    """
    加载论文1中的数据集
    """
    ...
    def preprocess_data(data):
        # 原始数据是 [batch, height, width, channels] 格式
...
def main():
    # 加载数据
    x_train, x_val, x_test, X_test_freq = load_data(envir, data_path='data')
