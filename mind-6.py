...
def build_encoder(com_qubits):
    ...
    # 递归编码函数
    def add_encoder(circ, index, current_index):
        if index == 1:
            circ += X.on(1)
            ...
            current_index = add_encoder(circ, index-1, current_index)
            circ += X.on(index)
            new_index = current_index ^ (1 << (index-1))
            circ += RY(com_parm[new_index]).on(0, range(1, com_qubits+1))
            current_index = add_encoder(circ, index-1, new_index)
            return current_index
    
    add_encoder(encoder, com_qubits, com_index)
    return encoder

def build_checker(img_qubits):
    img_parm = [f'img_{i}' for i in range(2**img_qubits)]       # 用于编码原始矩阵数据的量子线路的参数
    checker = Circuit()
    img_index = 2**(img_qubits-1)-1
    
    # 递归检查函数
    def add_checker(circ, index, current_index):
        if index == 1:
            new_index = current_index ^ 1
            ...
            current_index = add_checker(circ, index-1, current_index)
            new_index = current_index ^ (1 << (index-1))
            circ += RY(img_parm[new_index]).on(0, range(1, img_qubits+1))
            circ += X.on(index)
            current_index = add_checker(circ, index-1, new_index)
            return current_index
    
    add_checker(checker, img_qubits, img_index)
    checker += RY(img_parm[img_index]).on(0, range(1, img_qubits+1))
    checker += UN(H, range(1, img_qubits+1))
    return checker

# 构造压缩恢复量子线路
encoder = build_encoder(com_qubits)
encoder = encoder.no_grad()
encoder.as_encoder()

# 构造对比量子线路
checker = build_checker(img_qubits)
...
class Hybrid_Net(ms.nn.Cell):
    def __init__(self, img_channels, img_height, img_width, com_height, com_width):
        ...
        self.com_index = com_height * com_width * img_channels - 1                      # 定义量子恢复线路的参数
        self.com_qubits = int(np.log2(self.com_index+1))
        self.com_parm = [f'com_{i}' for i in range(2**self.com_qubits)]

        self.img_index = img_height * img_width * img_channels                          # 定义量子对比线路的参数
        self.img_qubits = int(np.log2(self.img_index))
        self.img_parm = [f'img_{i}' for i in range(2**img_qubits)]

        self.encoder = Circuit(n_qubits=(self.com_qubits+1))                            # 构建量子恢复线路
        self.ansatz = Ansatz10(total_qubits, n_layers).circuit.as_ansatz()              # 构建ansatz
        self.checker = Circuit(n_qubits=(self.img_qubits+1))                            # 构建量子对比线路
    ...
    def construct(self, x):
        self.com_parm = self.compressor(x)
        self.encoder += UN(H, range(1, self.com_qubits+1))
        self.encoder += RY(self.com_parm[self.com_index]).on(0, range(1, self.com_qubits+1))
        self.com_encoder(self.com_qubits)
        self.encoder = self.encoder.no_grad()
        self.encoder.as_encoder()

        self.img_checker(self.img_qubits) 
        self.checker += RY(self.img_parm[self.img_index]).on(0, range(1, self.img_qubits+1))
        self.checker += UN(H, range(1, self.img_qubits+1))
        self.checker = self.checker.no_grad()
        self.check.as_encoder()
...
# 创建混合模型
hybrid_model = Hybrid_Net(compressor, quantum_net)
