...
class Hybrid_Net(ms.nn.Cell):
    def __init__(self, img_channels, img_height, img_width, com_height, com_width):
        ...
        self.circuit = Circuit(n_qubits=(self.img_qubits+1))
    ...
    def construct(self, x):
        ...
        self.encoder += RY(compressed_x[self.com_index]).on(0, range(1, self.com_qubits+1))
        ...
        self.circuit = self.encoder + self.ansatz + self.checker
        return output


# 创建混合模型
hybrid_model = Hybrid_Net(img_channels, img_height, img_width, com_height, com_width)
print(hybrid_model.parameters_and_names())
...
grad_ops = sim.get_expectation_with_grad(hams, hybrid_model)
