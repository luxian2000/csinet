...
# 重写量子编码器函数，避免使用全局变量
def build_com_encoder(com_qubits, com_parm):
    encoder = Circuit()
...
def build_img_checker(img_qubits, img_parm):
    checker = Circuit()
...
# 构造压缩恢复量子线路
encoder = build_com_encoder(com_qubits, com_parm)
encoder = encoder.no_grad()
encoder.as_encoder()

# 构造对比量子线路
checker = build_img_checker(img_qubits, img_parm)
...
class HybridQuantumClassicNet(ms.nn.Cell):
    def __init__(self, compressor, quantum_net):
